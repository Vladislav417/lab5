...
x_aver_max = sum([x[1] for x in x_range]) / 3 x_aver_min = sum([x[0] for x in x_range]) / 3

y_max = 200 + int(x_aver_max) y_min = 200 + int(x_aver_min)


# квадратна дисперсія
def s_kv(y, y_aver, n, m): res = []
...
def add_sq_nums(x):
    for i in range(len(x)): x[i][4] = x[i][1] * x[i][2]
...
def kriteriy_cochrana(y, y_aver, n, m): f1 = m - 1
...
def cohren(f1, f2, q=0.05): q1 = q / f1
...
def kriteriy_studenta(x, y, y_aver, n, m): S_kv = s_kv(y, y_aver, n, m)
...
f_t = fisher(dfn=f4, dfd=f3)  # табличне знач print('\nПеревірка адекватності за критерієм Фішера') print('Fp =', F_p)
print('F_t =', f_t)
if F_p < f_t:
